...
const { data: roomsData } = await useFetch("api/v1/rooms/", {
	baseURL: "https://nuxr3.zeabur.app/",
	method: "GET",
});

const rooms = computed(() => roomsData.value?.result || []);
const showRoomImages = computed(() => rooms.value.map((r) => r.imageUrl));
</script>
...
<template>
	<main>
		<section class="hero position-relative">
			...
			>
				<swiper-slide v-for="image in showRoomImages" :key="index">
					<picture>
